...
try :
    N = int(input("Введите количество строк (столбцов) квадратной матрицы в интервале от 10 до 100: "))
    while N < 10 or N > 100:
        N = int(input("Вы ввели неверное число\nВведите количество строк (столбцов) квадратной матрицы в интервале от 10 до 100:"))
    K = int(input("Введите число К="))
    start = time.time()

    Obl1, Obl3, = [], []     #задаем необходимые величины
    A, F, AF, AT = [], [], [], []
    ###################################################
    for i in range(N):
        A.append([0]*N)
    ...
    time_prev = time_next
    time_next = time.time()
    for i in range(N):     #формируем матрицу F
        for j in range(N):
            F[i][j] = A[i][j]
    time_prev = time_next
    time_next = time.time()
    ...
    #################
    for i in range(n1):   # обрабатываем подматрицу E
        for j in range(n1):
            if i < j and j < n1 - 1 - i and j % 2 == 0:
                Obl1.append(E[i][j])
    minima = min(Obl1)
    print("Область 1 в нечетных столбцах состоит из :",Obl1)
    ...
    for i in range(n1):   # обрабатываем подматрицу E
        for j in range(n1):
            if i > j and j > n1 - 1 - i and i % 2 == 0:
                Obl3.append(E[i][j])
    summ = np.sum(Obl3)
    print("Область 3 в нечетных строках состоит из :",Obl3)
    ...
    if minima > summ:  #если минимальный элемент > суммы элементов
        ...
        print ("Минимальный элемент в области 2 > суммы элементов в области 3 => меняем области 2 и 3 местами в подматрице B симметрично \n")
       ...
       print("Минимальный элемент в области 2 < суммы элементов в области 3 => меняем подматрицы B и E местами нессиметрично \n")
